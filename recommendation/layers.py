...
class Dense2(nn.Module):
    def __init__(self, input_dim, output_dim, dropout=0, chnl=8):
...
class CNN_cross( nn.Module ):
    ...
    def __init__( self, dim):
        ...
        self.ConV = nn.Conv2d(in_channels=1, out_channels=2, kernel_size=(5,1))
    def forward(self, input):
        ...
        Z9 = torch.zeros(HI.shape[0], 1, 8)
        # Z11 = torch.zeros(HI.shape[0], 1, 10)
        ...
        HI7 = HI7[:, :, perm7]
        perm9 = torch.randperm(HI9.size(-1))
        HI9 = HI9[:, :, perm9]
        # HI11 = torch.cat([HI11, Z11], dim=2)
        # HI13 = torch.cat([HI13, Z13], dim=2)
        HIcat=torch.cat([HI1,HI3,HI5,HI7,HI9],dim=1)
...
class GCN( nn.Module ):
    ...
    def __init__( self, dim,batch_size):
        ...
        self.T_Embedding = nn.Embedding(6729, self.dim)
        self.R_Embedding=nn.Embedding(7,self.dim)
        self.U_Embedding = nn.Embedding(6036, self.dim)
    def forward(self, input):
        u,v=input
        kg_data = pd.read_table('D:\Pycharm\推荐系统\Test\KGCN-master\MKR.PyTorch-master\data\movie\kg_final.txt',
...
class HCN( nn.Module ):
    ...
    def __init__( self, dim,batch_size):
        ...
        self.batch_size=batch_size
        self.T_Embedding = nn.Embedding(6729, self.dim)
        self.H_Embedding = nn.Embedding(2347, self.dim)
        self.R_Embedding=nn.Embedding(7,self.dim)
    def forward(self, input):
        h=input
        h=h[0]
